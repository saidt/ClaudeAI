...
type
  ...
  private
    ...
    procedure ResetAutoFieldsReadOnly;
  ...
  public
    ...
    procedure Refresh;
  published
    // Republished (TDataSet declares it public, not published) so it
    // shows in the Object Inspector and, more importantly, so the IDE's
    // Fields Editor can connect at design time (Connection + BaseSQL set,
    // Active toggled on in the IDE) and let you "Add fields..." to create
    // persistent, strongly-typed TField objects for this dataset, exactly
    // as with TFDQuery/TTable.
    property FieldDefs;
...
procedure TVPFDPagedDataSet.ResetAutoFieldsReadOnly;
var
  i: Integer;
begin
  // FireDAC's schema probe wraps BaseSQL as a derived table
  // (SELECT TOP 0 * FROM (BaseSQL) x), and through that wrapper it
  // typically cannot prove any column maps back to an updatable base
  // table column, so it reports every column as read-only metadata -
  // which CreateFields then turns into Field.ReadOnly := True. That
  // verdict is irrelevant here: CRUD is generated by us, against
  // TableName/KeyFields (ExecuteInsert/ExecuteUpdate/ExecuteDelete), not
  // by FireDAC's own automatic-update detection. Left uncorrected, this
  // silently blocks grid editing (and, since IsWritableField also
  // consults Field.ReadOnly, silently drops every column from generated
  // INSERT/UPDATE statements too). Only auto-created fields are touched -
  // persistent (design-time) fields keep whatever ReadOnly the developer
  // set explicitly in the Fields Editor / Object Inspector.
...
begin
  ...
  // Never discard/reload the page cache while a row in it is actively
  // being edited or inserted - e.g. a grid's background row-lookahead
  // asking for a neighbouring page mid-edit. Rebuilding FPageCache here
  // would corrupt the in-progress edit (FPageCache.Delete/CreateDataSet
  // while FPageCache.State is dsEdit/dsInsert raises). Report the row as
  // unavailable instead; real user navigation is already blocked by
  // TDataSet.CheckBrowseMode while editing, so this only ever guards an
  // internal prefetch, never an actual requested scroll.
  if FPageCache.State in [dsEdit, dsInsert] then
    Exit(False);
